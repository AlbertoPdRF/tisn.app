...
const createUser = async (
  name, email, emailConfirmed, emailConfirmedAt,
    dateOfBirth, country, region,
    preferredLocale, interests, admin,
    callback
) => {
  const user = new User({
    name, email, emailConfirmed, emailConfirmedAt,
...
const generateUsersArray = async () => {
  ...
  let arr = []
  for (let i = 0; i < 100; i++) {
    const name = `${uniqueNamesGenerator(config)} ${uniqueNamesGenerator(config)}`;
    const email = `${name.replace(/ /g, '_')}@test-tisn-email.com`.toLowerCase();
    ...
    const birthDate = randomDate(new Date(1950, 01, 01), new Date(2007, 01, 01));
    interests = interests.sort(() => 0.5 - Math.random());
    const selectedInterests = interests.slice(0, Math.floor(Math.random() * 10));
